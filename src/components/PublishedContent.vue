<template>
  <article class="post-wrapper my-2 flex flex-wrap flex-shrink-0 justify-center">
    <div v-if="post" class="post max-w-xl bg-custom text-white p-3 rounded-3xl mx-2">
      <img :src="`http://localhost:3000/image/${post.coverImage.filename}`" />
...
export default {
  ...
  async mounted () {
    const responseData = await axios.get('http://localhost:3000/blog-content/' + this.$route.params.id)
    this.post = responseData.data.results
    const postsData = await axios.get('http://localhost:3000/all-posts/')
